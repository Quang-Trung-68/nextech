...
import { adminPaths, isNewsSection } from '@/configs/adminPaths';
...
const Sidebar = ({ isMobile }) => {
  ...
  const links = [
    { name: 'Tổng quan', to: adminPaths.overview, icon: LayoutDashboard },
    { name: 'Sản phẩm', to: adminPaths.products, icon: Package },
    { name: 'Đơn hàng', to: adminPaths.orders, icon: ShoppingCart },
    { name: 'Nhà cung cấp', to: adminPaths.inventorySuppliers, icon: Warehouse },
    { name: 'Nhập kho', to: adminPaths.inventoryStockImports, icon: Truck },
    { name: 'Serial / IMEI', to: adminPaths.inventorySerials, icon: Barcode },
    { name: 'Người dùng', to: adminPaths.users, icon: Users },
    { name: 'Mã giảm giá', to: adminPaths.coupons, icon: Tag },
    { name: 'Tin tức', to: adminPaths.news, icon: Newspaper },
    { name: 'Banner', to: adminPaths.banners, icon: Image },
    { name: 'Thương hiệu', to: adminPaths.brands, icon: Award },
    { name: 'Cài đặt', to: adminPaths.settings, icon: Settings },
  ];
      ...
      <nav className="flex flex-1 flex-col gap-1">
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
            end={link.to === adminPaths.overview}
            title={!isMobile && collapsed ? link.name : undefined}
            className={({ isActive }) => {
              let active = isActive;
              if (link.to === adminPaths.news) {
                active = isNewsSection(pathname);
              }
              return cn(
