...
- **CI/CD (GitHub Actions + ghcr.io):** workflow [`.github/workflows/ci-cd.yml`](./.github/workflows/ci-cd.yml) — lint → build & push 3 image (`nextech-backend`, `nextech-soketi`, `nextech-frontend`) → SSH deploy (pull, `up --no-build`, `prisma generate`, `prisma migrate deploy`). Trên VPS, **`GHCR_PREFIX` trong `.env` gốc là bắt buộc** — `ghcr.io/<GitHub owner chữ thường>` trùng chỗ workflow push image (vd. `github.com/MyOrg/repo` → `GHCR_PREFIX=ghcr.io/myorg`). Không dùng placeholder `your-github-org`; thiếu biến này `docker compose` sẽ báo lỗi. Cấu hình Secrets trong repo (xem workflow). Deploy thủ công vẫn dùng `bash scripts/deploy.sh` (build local).
