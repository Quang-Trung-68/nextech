  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (data.paymentMethod === "STRIPE") {
        ...
        // 2. Stripe Confirm
        const { error: stripeConfirmErr, paymentIntent } =
        ...
        if (paymentIntent && paymentIntent.status === "succeeded") {
          ...
          navigate(`/profile/orders/${orderId}?success=true`, {
            replace: true,
          });
        }
        ...
        if (
          ...
          !response.sepayFields
        ) {
          throw new Error(
            response.message || "Lỗi thiết lập thanh toán SePay.",
          );
        }
        ...
        queryClient.invalidateQueries({ queryKey: ["cart"] });
        navigate(`/profile/orders/${response.order.id}?success=true`, {
          replace: true,
        });
      }
    } catch (error) {
  ...
  return (
    <div className="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 py-8 md:py-12 min-h-svh flex flex-col">
      ...
      <div className="flex items-center space-x-2 text-apple-secondary mb-6 text-xs md:text-sm font-medium">
        ...
        <span className="text-apple-blue font-bold shrink-0">
          2. Thanh toán
        </span>
        <span className="text-[#d2d2d7] shrink-0">—</span>
      ...
      <h1 className="text-3xl md:text-4xl font-extrabold text-apple-dark tracking-tight mb-8">
        Thanh toán an toàn{" "}
        <Lock className="inline w-6 h-6 text-apple-secondary mb-1 opacity-50" />
      </h1>
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <div className="lg:hidden w-full">
            <Accordion
              type="single"
              collapsible
              className="w-full bg-apple-gray border border-[#d2d2d7] rounded-xl px-4 py-1"
            >
          ...
          {/* ─── Coupon Section ─────────────────────────────────────────── */}
          <div className="bg-white p-4 md:p-6 rounded-2xl border border-border">
            <h2 className="text-base font-bold tracking-tight text-foreground mb-3">
              Mã giảm giá
            </h2>
            <CouponInput
